...
class Stats(object):
    ...
    def gather_value(self,val,mean=False):
    ...
    def update(self,preds,time_start=None,freeze_iter=False,stat_set='train'):
        ...
        for stat in self.log_vars:
            ...
            if stat=='sec/it': # compute speed
                ...
                if stat in preds:
                    try:
                        if stat[:5].lower() == 'eval_' or stat[:4].lower() == 'acc_':
                            val = self.gather_value(preds[stat], mean=True)
                        else:
    ...
    def get_epoch_averages(self,epoch=None):
        ...
        for stat_set in stat_sets:
            outvals[stat_set] = { 'epoch':epoch, \
                'it':self.it[stat_set], 'epoch_max':self.epoch }
    ...
    def plot_stats( self, visdom_env=None, plot_file=None, \
        ...
        if not novisdom:
            for idx , ( tmodes, stat , x , vals ) in enumerate( lines ):
                ...
                # print ("Adding: {0}, (X = {1}, y = {2})".format(title, vals, x))

                # bjb_edit: This seems necessary to deal with (n,) vs (n,1) array differences
                try:
