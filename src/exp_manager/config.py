...
def get_arg_parser(cfg_constructor, default=None):
    
    dargs = get_default_args(cfg_constructor)
    if default is not None:
        dargs.update(default)       
...
def get_config_from_file(cfg_filename): 
    # set config from yaml file
    with open(cfg_filename, 'r') as f:
        # yaml_cfg = yaml.load(f)
        yaml_cfg = yaml.safe_load(f)
    return convert_to_stringval(yaml_cfg,stringify_vals=False)
