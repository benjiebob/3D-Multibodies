...
def fix_color(color_np):
    return tuple ([int(x) for x in color_np])
...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    if joints.shape[1] == 19:
        ...
        print('Unknown skeleton!! Input size: {0}'.format(joints.shape))
        parents = None
    ...
    if parents is not None:
        for child in range(len(parents)):
            ...
            if vis is not None and vis[child] == 0:
                continue
            if draw_edges:
                cv2.circle(image, (point[0], point[1]), radius, fix_color(colors['white']), -1)
                cv2.circle(image, (point[0], point[1]), radius - 1, fix_color(colors[jcolors[child]]), -1)
            else:
            ...
            if draw_edges and pa_id >= 0:
                ...
                cv2.circle(image, (point_pa[0], point_pa[1]), radius - 1, fix_color(colors[jcolors[pa_id]]), -1)
                if child not in ecolors.keys():
                    print('bad')
                    import ipdb
                    ipdb.set_trace()
                cv2.line(image, (point[0], point[1]), (point_pa[0], point_pa[1]), fix_color(colors[ecolors[child]]), radius - 2)
