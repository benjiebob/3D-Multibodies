...
def train_data(dataset_path, openpose_path, out_path, joints_idx, scaleFactor, extract_img=False, fits_3d=None):
    ...
    for user_i in user_list:
        print ("-> Running User: {0} of {1}".format(user_i, len(user_list)))
        for seq_i in seq_list:
            print ("->-> Running seq: {0} of {1}".format(seq_i, len(seq_list)))
            ...
            for j, vid_i in enumerate(vid_list):
                print("->->-> Running video: {0} of {1}".format(j, len(vid_list)))
                ...
                if extract_img:
                    ...
                    vid_framerate = int(vidcap.get(cv2.CAP_PROP_FPS))

                    print ("-> Extracting {0} frames of {1} into {2}. FPS={3}".format(vid_length, j, imgs_path, vid_framerate))
                    pbar = tqdm(total=vid_length, desc=os.path.basename(imgs_path))
                    ...
                    while 1:
                        ...
                        # print ("Writing -> {0}".format(imgname))
    ...
    out_file = os.path.join(out_path, 'mpi_inf_3dhp_train2.npz')
