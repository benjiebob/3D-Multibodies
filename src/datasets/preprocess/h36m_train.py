...
def h36m_train_extract(dataset_path, openpose_path, out_path, extract_img=False):
    ...
    for user_i in user_list:
        ...
        bbox_path = os.path.join(
            dataset_path, user_name, "MySegmentsMat", "ground_truth_bb"
        )
        # path with GT 3D pose
        pose_path = os.path.join(
            dataset_path, user_name, "MyPoseFeatures", "D3_Positions_mono"
        )
        # path with GT 2D pose
        pose2d_path = os.path.join(
            dataset_path, user_name, "MyPoseFeatures", "D2_Positions"
        )
        # path with videos
        ...
        for seq_i in seq_list:
            ...
            for frame_i in range(poses_3d.shape[0]):
                ...
                if frame_i % 5 == 0 and (protocol == 1 or camera == "60457274"):
                    ...
                    bbox = np.array(
                        [np.min(xs), np.min(ys), np.max(xs) + 1, np.max(ys) + 1]
                    )
                    center = [(bbox[2] + bbox[0]) / 2, (bbox[3] + bbox[1]) / 2]
                    scale = 0.9 * max(bbox[2] - bbox[0], bbox[3] - bbox[1]) / 200.0
                    ...
                    # read GT 3D pose
                    Sall = np.reshape(poses_3d[frame_i, :], [-1, 3]) / 1000.0
                    S17 = Sall[h36m_idx]
